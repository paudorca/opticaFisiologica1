...
function downloadFile() {
	...
	var aux = 2.9*distance/i; 
	var porcentaje = parseFloat((correctAnswers[i]/totalAnswers[i])); 
	myArray.push({"V.A":i/10,"s(mm)":aux.toFixed(3),"Distance(m)":distance,
	"Correct answers":correctAnswers[i],"Total answers":totalAnswers[i],
	"Percentage(%)":toPercent(porcentaje)});
}
	arrayObjToCsv(myArray);
}

function toPercent(point){
	var percent = Number(point*100).toFixed(0);
	return percent;
}